package com.dofun.sxl.constant;

public class EventConstants {
    /**
     * 诵经典 courseId:10
     */
    public static final int SJD_TK = 101;//填空
    public static final int SJD_LX = 102;//连线
    public static final int SJD_PS = 103;//拼诗
    public static final int SJD_XZ = 104;//选择
    public static final int SJD_PD = 105;//判断
    public static final int SJD_SD = 106;//诵读
    public static final int SJD_SP = 107;//视频

    /**
     * 练运算 courseId:11
     */
    public static final int LYS_KS = 111;//口算
    public static final int LYS_JS = 112;//计算
    public static final int LYS_TK = 113;//填空
    public static final int LYS_XZ = 114;//选择

    public static final int LYS_POSITION = 110;//练运算小题的位置

    //更换头像
    public static final int USER_IMG = 1001;

    //finish界面
    public static final int FINISH = 10000;


}
